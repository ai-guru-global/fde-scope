...
QwenPaw 桌面端（Tauri）
 └─ App Center → FDE Scope PawApp
     ├─ backend/main.py   PawApp SDK：FastAPI 路由由宿主挂在 /api/fde-scope/*
     │     └─ 薄封装 fde_scope 包（engagement/gates/corpus/skills/llm）
     └─ ui/index.js       运行时加载的 React 页面（/apps/fde-scope）
```

- **后端**：`PawApp(app_id="fde-scope")` + FastAPI router，由宿主注册在
  `/api/fde-scope` 前缀下（与 scripts/verify_pawapp_host.py 的真实装载一致）。
...
- **Agent 工具**：注册了 `fde_sop_status` / `fde_sop_advance` /
  `fde_deploy_plan`，QwenPaw 的 Agent 可以在对话中查询/推进 SOP 状态机、
  预演角色 Agent 的连接器工具绑定。
...
| POST `/deploy/plan` | 预演 `fde-scope deploy`：角色 → 连接器 → 工具绑定 + 缺失源清单（纯数据，不装配不调模型） |
...
- [x] `/deploy/plan` 路由 + `fde_deploy_plan` 工具：与 Web 控制台/CLI 共享
      `fde_scope.deploy.build_deploy_plan` 同一条代码路径
