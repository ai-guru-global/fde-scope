...
- **3 个使用入口**：Typer CLI（`fde-scope` 命令，全函数内延迟导入）、FastAPI Web 控制台（26 路由，三视图）、QwenPaw PawApp 桌面插件（17 路由挂载 `/api/fde-scope`，薄封装复用宿主 LLM/沙箱/存储）。
- **1 个核心引擎包 `fde_scope`**：SOP 层（engagement + gates）、场景层（profiles）、能力层（connectors/corpus/deploy/eval/flywheel/integrations/skills）、横切层（llm/config/templates）。
- **1 个零数据库文件事实源 `.fde_scope/`**：engagement 快照、技能库（draft→published→archived）、uploads；`reports/` 存 HTML 报告与 runbook。
- **3 个可选外部依赖**：MiMo LLM（标准库 urllib、失败回退规则）、AgentScope 2.0.5（optional extra，**仅 `deploy/` 延迟导入**）、QwenPaw 宿主。

## 关键架构事实（均有证据）

1. **零配置可跑**：核心层无 agentscope 依赖，`pip install -e ".[dev]"` + pytest 全绿（全库导入扫描验证）。
...
- 固化架构守护测试（阶段数=18、gate 数=10、`import agentscope` 仅限 deploy/），见健康报告。
