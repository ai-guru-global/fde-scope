workspace "FDE Scope" "FDE Scope 现状架构模型（current-state，证据见 system-model.evidence.md）" {
    ...
    model {
        ...
        agentscope = softwareSystem "AgentScope 2.0.5" "可选运行时（optional extra）。仅 deploy/ 延迟导入：DockerWorkspace/Agent/PermissionEngine/SubAgentTemplate" {
            tags "External"
        }
        ...
        fdeScope = softwareSystem "FDE Scope" "FDE 现场操作系统：18 阶段 SOP 状态机 + 10 个可执行 gate，覆盖 ticket 与 manufacturing 双场景" {
            ...
            webConsole = container "Web 控制台" "FastAPI 三视图工作台（workbench 聚合 + journal + skills），26 个路由" "Python / FastAPI (optional extra web)"

            pawapp = container "PawApp 插件应用" "QwenPaw 桌面形态：后端薄封装 17 路由挂载 /api/fde-scope + 2 个 Agent 工具 + skill_provider" "Python / QwenPaw PawApp SDK"

            coreEngine = container "核心引擎 fde_scope" "纯 Python 库，核心层零 agentscope 依赖。engagement/connectors/corpus/deploy/eval/flywheel/integrations/profiles/skills + 横切 llm/config/templates" "Python package"

            fileStore = container "文件事实源 .fde_scope/" "零数据库文件库：engagements/（SOP 状态快照）、skills/（技能 draft→published→archived）、uploads/；reports/ 存 HTML 报告与 runbook" "JSON / JSONL / Markdown / HTML files"
        }
        ...
        coreEngine -> agentscope "仅 deploy/ 延迟导入（optional extra [agentscope]）"
