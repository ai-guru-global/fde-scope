...
> 本 catalog 已有 3 个 📦 条目的**真实来源就是本仓库**：[sre-runbooks](../zone-c-operationalization/sre-runbooks.md)（真名 `runbook`）· [incident-response](../zone-c-operationalization/incident-response.md) · [anthropic-documentation](../zone-d-handoff/anthropic-documentation.md)（真名 `documentation`）
...
| 交付文档写作规范 | `documentation` | 8.8K | Zone D（已有页，见上） |
...
| 事件响应与 runbook | `incident-response` / `runbook` / `ticket-triage` | 5.3K / 2.6K / 2.6K | Zone C（前两页已建） |
| 上线清单 | `deploy-checklist` | 4.9K | Zone B→C 交界，正好补 gate 证据模板 |
...
## 项目应用位点
- 若只装 3 个的推荐顺序：`deploy-checklist`（直接补 gate 证据模板）→ `runbook`（Zone C 交付物）→ `documentation`（Zone D 写作规范）
