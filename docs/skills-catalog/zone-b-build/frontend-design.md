...
- **触发**：让 agent 构建网页/组件/落地页/dashboard/海报或"美化任何 UI"时适用（skill description），项目里典型话术：「给 demo 工作台出一版有辨识度的界面」
- **第一步**：先给设计 brief 再让它写码——受众、情绪/参考、页面的唯一任务；skill 流程第一步是产出 token 系统：4–6 个命名 hex 色板、display/body 字体搭配、布局 ASCII 草图、一个 signature 记忆点，再进代码
- **常见坑**：skill 点名三种"AI 默认脸"（暖米底+衬线+terracotta 点缀、近黑底+荧光绿/朱红单点缀、报纸式 hairline 排版）——不给 brief 任其发挥大概率落进这三款之一
- **常见坑**：别引入需联网的字体/CDN：air-gap 现场断网资源加载失败，比丑更致命（页面 Don't）；动效按 skill 建议克制编排，别抢演示焦点
