...
- ONTO-002  sub_property_of / inverse 引用含环
...
def resolve_imports(schema: OntologySchema, loader: Loader) -> OntologySchema:
    ...
    order: list[OntologySchema] = []
    done: set[str] = set()
    visiting: set[str] = set()

    def visit(cur: OntologySchema) -> None:
    ...
    for cur in order:  # 依赖先入，自身后入 → 自身声明覆盖依赖
